...
def train_one_epoch(model, train_loader, criterion, optimizer, device, scaler, 
                    supcon_loss_cbp=None, supcon_loss_gem=None, max_grad_norm=2.0, use_crop=False):
    model.train()
    running_loss = 0.0
    correct_preds = 0
    total_preds = 0
    ...
    for images, labels in pbar:
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16): 
            if use_crop:
                ...
                loss_cbp_full_ce = criterion(logits_cbp_full, labels, weight_cbp)
                loss_gem_full_ce = criterion(logits_gem_full, labels, weight_gem)
                
                # ⭐ SupCon 特徵流形推拉
                loss_cbp_supcon_full = supcon_loss_cbp(embed_cbp_full, labels)
                loss_gem_supcon_full = supcon_loss_gem(embed_gem_full, labels)

                loss_cbp_full = loss_cbp_full_ce + 0.1 * loss_cbp_supcon_full
                loss_gem_full = loss_gem_full_ce + 0.1 * loss_gem_supcon_full
                ...
                # --- 階段 3：局部特徵圖 Pass ---
                logits_cbp_crop, logits_gem_crop, embed_cbp_crop, embed_gem_crop = model(cropped_imgs)
                
                loss_cbp_crop_ce = criterion(logits_cbp_crop, labels, weight_cbp)
                loss_gem_crop_ce = criterion(logits_gem_crop, labels, weight_gem)
                
                # ⭐ 裁切圖的 SupCon 特徵流形推拉
                loss_cbp_supcon_crop = supcon_loss_cbp(embed_cbp_crop, labels)
                loss_gem_supcon_crop = supcon_loss_gem(embed_gem_crop, labels)

                loss_cbp_crop = loss_cbp_crop_ce + 0.1 * loss_cbp_supcon_crop
                loss_gem_crop = loss_gem_crop_ce + 0.1 * loss_gem_supcon_crop
                ...
                # ⭐ SupCon 特徵流形推拉
                loss_cbp_supcon = supcon_loss_cbp(embed_cbp, labels)
                loss_gem_supcon = supcon_loss_gem(embed_gem, labels)

                loss_cbp = loss_cbp_ce + 0.1 * loss_cbp_supcon
                loss_gem = loss_gem_ce + 0.1 * loss_gem_supcon
