...
def plot_long_tail_accuracy(train_labels, val_preds, val_labels, num_classes=100, save_path="./Plot/long_tail_acc.png"):
    ...
    color = 'skyblue'
    ax1.set_xlabel('Classes Sorted by Training Frequency (Head -> Tail)', fontsize=12)
    ax1.set_ylabel('Number of Training Images', color=color, fontsize=12)
    ax1.bar(range(num_classes), sorted_train_counts, color=color, alpha=0.6, label='Train Sample Count')
    ax1.tick_params(axis='y', labelcolor=color)
  
    ax1.set_xticks(range(0, num_classes, 5))
    ax1.set_xticklabels([f"#{sorted_indices[i]}" for i in range(0, num_classes, 5)], rotation=45, fontsize=8)

    
    ax2 = ax1.twinx()
    color = 'red'
    ax2.set_ylabel('Validation Accuracy (%)', color=color, fontsize=12)
    ax2.plot(range(num_classes), sorted_val_accs, color=color, marker='o', markersize=4, linestyle='-', linewidth=2, label='Val Accuracy')
    ax2.tick_params(axis='y', labelcolor=color)
    ax2.set_ylim(-5, 105)


    plt.title('Long-Tail Diagnostic: Accuracy vs. Class Frequency', fontsize=14, fontweight='bold')
    fig.tight_layout()
    plt.grid(True, linestyle='--', alpha=0.5)
